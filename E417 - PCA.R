...
### Select data to plot
title = 'E401-all data'
title = 'E401- wo iPSCs'; ang.metadata <- ang.metadata[1:9,]      # Everything but iPSCs
...
(pca.1.v.2 <- ggplot(data = pca.data.to.plot, aes(x = PC1, y = PC2, label = Shortname)) +
    ...
    xlim(min(pca.data.to.plot$PC1)-5,max(pca.data.to.plot$PC1+5)) +
...
(pca.2.v.3 <- ggplot(data = pca.data.to.plot, aes(x = PC2, y = PC3, label = Shortname)) +
    geom_point(size = 5, aes(fill = Condition), color = 'black', pch = 21, stroke = 2) +
    #geom_text(hjust=-0.2,vjust=0.5) + 
    xlim(min(pca.data.to.plot$PC2)-5,max(pca.data.to.plot$PC2+5)) +
    scale_fill_manual(values = c("#fe1c1c", "#fab9b6", "#a6acf7", "#000dc4")) +
    labs(x = paste('PC2 - ', pca.data.var.per[2], '%', sep = ''), 
         y = paste('PC3 - ', pca.data.var.per[3], '%', sep = '')) +
    theme(plot.title = element_text(color="black", face="bold", size=22, margin=margin(10,0,20,0)),
          axis.title.x = element_text(face="bold", size=14,margin =margin(20,0,10,0)),
          axis.title.y = element_text(face="bold", size=14,margin =margin(0,20,0,10)),
...
tiff(filename= paste0(title, ' - PCA 2v3.tiff'), width = 2000, height = 1600, units = "px", pointsize = 12, res = 250)
pca.2.v.3
dev.off()

tiff(filename= paste0(title, ' - PCA 1v4.tiff'), width = 2000, height = 1600, units = "px", pointsize = 12, res = 250)
pca.1.v.4
dev.off()

tiff(filename= paste0(title, ' - PCA 1v5.tiff'), width = 2000, height = 1600, units = "px", pointsize = 12, res = 250)
pca.1.v.5
dev.off()
